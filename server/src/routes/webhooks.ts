  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        const existingOrders: any[] = await tx.$queryRaw`SELECT "paymentStatus" FROM "Order" WHERE "stripePaymentIntentId" = ${paymentIntentId} FOR UPDATE`;
     ...
     try {
       await prisma.$transaction(async (tx) => {
         const existingOrders: any[] = await tx.$queryRaw`SELECT id, "paymentStatus" FROM "Order" WHERE "stripePaymentIntentId" = ${paymentIntentId} FOR UPDATE`;
